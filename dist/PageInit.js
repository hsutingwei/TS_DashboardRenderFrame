...
class SearchOperation {
    ...
    static ClickSearch(tPageName, qyStr) {
        ...
        if (tPageNumber > -1) {
            ...
            let tTableTitle = gPageObj.PageNameObj[tPageName].TitleStrArr;
    ...
    static Search(tPageName, JumPage) {
        ...
        if (PageNumber >= 1) {
            ...
            if (tmpTitle.length > 0) {
                AttributeStr = 'id="' + TableIdName + '" class="hover row-border cell-border order-column table table-striped" style="width:100%"';
            }
            let TableHtml = pm.CreatReadWriteTable(tmpPageName, [], AttributeStr, tmpTitle);
            ...
            let ttt = '<table id="' + tmpPageName + 'Table"></table>';
            ...
            TableObj.createdRow = function (row, data, index) {
                ...
                for (let i = 0; $('td', row).eq(i).html(); i++) {
                ...
                for (let i = 0; i < ValueIdArr.length; i++) {
                    let tmpSelectList = pm.FrontDynamicMenuRequest(tmpPageName, tFieldNameArr[i], ValueIdArr[i], KeyValueArr[i]);
            ...
            doAjax('Search', true, Query, function (data) {
                ...
                for (let i = 0; i < gPageObj.PageNameObj[tmpPageName].TitleStrArr.length; i++) {
                    if (set.TableSetObj.MillionFieldArr.indexOf(gPageObj.PageNameObj[tmpPageName].TitleStrArr[i]) > -1 && UnitMode != '數量') {
                        HaveMillion = true;
                        break;
                    }
                }
...
export class TableAndSearchOperation extends SearchOperation {
    ...
    static AddRowInitList(tPageName) {
        ...
        for (let i = 0; i < ValueIdArr.length; i++) {
            let tmpSelectList = pm.FrontDynamicMenuRequest(tPageName, tFieldNameArr[i], ValueIdArr[i], KeyValueArr[i]);
            let domId = ValueIdArr[i];
            let tIdx = Number(domId.substring(domId.lastIndexOf('_') + 1));
...
export class PageMake {
    CreatReadWriteTable(tPageName, data, AttributeStr, TitleArr) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            for (let j = 0; j < ValueIdArr.length; j++) {
                let tmpSelectList = this.FrontDynamicMenuRequest(tPageName, tFieldNameArr[j], ValueIdArr[j], KeyValueArr[j]);
    ...
    MakeWidthAttributeStr(tPageName, InputFieldName, StyleAttr) {
        ...
        let tmpList = ps.GetListArr(tPageName, InputFieldName, true);
    ...
    InitSearchArea(tPageName) {
        ...
        /*let UrlGetObj = UrlGetVariable();
        tmpObj = SetDefaultValue(tmpObj, UrlGetObj);*/
        ...
        for (let i = 0; i < ValueIdArr.length; i++) {
            ...
            if (DefaultIdx > -1) {
                let tmpSelectList = this.FrontDynamicMenuRequest(tPageName, tFieldNameArr[i], ValueIdArr[i], KeyValueArr[i]);
                let domId = ValueIdArr[i];
    ...
    //KeyQuery: 下拉選單的值
    FrontDynamicMenuRequest(tPageName, FieldName, FieldId, KeyQuery) {
        var _a, _b;
        let dc = new set.DynamicClass();
        let ps = new set.PageSet();
        let valueArr = [];
        if (dc.DynamicInfObj[tPageName] && dc.DynamicInfObj[tPageName].InfluenceToFieldNames && Object.keys(dc.DynamicInfObj[tPageName].InfluenceToFieldNames).length > 0) {
            ...
            if (dc.DynamicInfObj[tPageName].InfluenceToFieldNames[FieldName][FieldId] != null) {
                ...
                if (!isNaN(tfIdx) && tFieldArr != null && valueArr != null) {
                    ...
                    valueArr = ps.GetListTitle(tPageName, tFieldArr[tfIdx], true, valueArr);
                }
            }
        }
        return valueArr;
        ...
        for (let i = 0; i < tdata.length; i++) {
            ...
            for (let j = 0; j < tmpArr.length; j++) {
                ...
                if (tmpSelectList.length > 0 && !ps.NoChangeField(gPageObj.PageNameObj[tPageName].TitleStrArr[j], tPageName, tmpArr[j])) {
                    tReArr.push(this.GetListValue(tmpSelectList, tmpArr[j]));
                }
                else {
                    tReArr.push(tmpArr[j]);
                }
