import { NeedAjaxArr, gPageObj, PageInf, PageMake, PageTool } from './PageInit.js';
...
export class PPSearch {
        ...
        switch (tPageName) {
            case 'AEIndex':
                $('#Block2').unbind().click(function () {
                    ...
                    let getStr = encodeURI('BU=' + tBu) + '&' + encodeURI('EQ Type=' + tType);
                    if (tBu == 'FT') {
                ...
                $('#Block4').unbind().click(function () {
                    ...
                    let getStr = encodeURI('BU=' + tBu) + '&' + encodeURI('機型=' + tType) + '&' + encodeURI('項目=' + tItem);
                    if (tBu == 'DS') {
            ...
            case 'PEIndex':
                ...
                $('#Block6').unbind().click(function () {
                    let tBu = gPageObj.PageNameObj[tPageName].LastQuery.QueryArr[0];
                    let tType = gPageObj.PageNameObj[tPageName].LastQuery.QueryArr[1] == '8@12' ? '' : gPageObj.PageNameObj[tPageName].LastQuery.QueryArr[1];
                    let getStr = encodeURI('BU=' + tBu) + '&' + encodeURI('機型=' + tType);
                    if (tBu == 'DS') {
            ...
            case 'QEIndex':
                ...
                $('#Block6').unbind().click(function () {
                    let tBu = gPageObj.PageNameObj[tPageName].LastQuery.QueryArr[0];
                    let tType = gPageObj.PageNameObj[tPageName].LastQuery.QueryArr[1] == '8@12' ? '' : gPageObj.PageNameObj[tPageName].LastQuery.QueryArr[1];
                    let getStr = encodeURI('BU=' + tBu) + '&' + encodeURI('機型=' + tType) + '&' + encodeURI('重點客戶勾選=true') + '&' + encodeURI('圖表類型=客戶');
                    window.open('http://192.168.5.141:83/Yield/Default/YieldRate?' + getStr);
                });
                break;
            default:
    ...
    BlockSearch(tPageName) {
        ...
        let oriPageName = tPageName;
        let PPISetChildPageName = function (tPageName, oriPageName) {
            var _a;
            if (gPageObj.PageNameObj[tPageName] == null) {
                ...
                if (((_a = gPageObj.PageNameObj[oriPageName]) === null || _a === void 0 ? void 0 : _a.SubBlockId) != null) {
        ...
        switch (tPageName) {
            ...
            case 'AE_CIRCLE':
                ...
                $('#Block1_1').unbind().click(function () {
                    ...
                    let getStr = encodeURI('BU=' + tBu) + '&' + encodeURI('區間=' + tMode) + '&' + encodeURI('年=' + tYear)
...
export class PPMake {
     ...
     */
    CheckDataCount(data, RowCount, CellCount) {
        if (data.length == 0) {
            return data;
        }
        for (let i = 0, m = 0, k = 1; i < data.length && k < RowCount; i = m) {
            let tN = Number(data[i].split(',')[data[i].split(',').length - 1]);
            if (tN != k) {
                let tStr = '';
     ...
     * @param {string[]} data 原完整搜尋結果陣列
     */
    OEEReport(tPageName, IdName, data) {
        let DomId = IdName + '_1';
        if (data.length == 0) {
        ...
        let htmlStr = '<div class="flex-1 flex-grow min-h-[calc((100vh-70px)*92/100*3/11)]" id="' + DomId + '_1"></div><div class="flex-1" id="' + DomId + '_2"></div>';
        let tmpData = [];
    ...
    ChartsOption(tPageName, data, idName) {
        if (gPageObj.PageNameObj[tPageName] == null) {
            return {};
        }
        let option = {};
        ...
        if ((tPageName == 'OEE_CP' || tPageName == 'OEE_LCD' || tPageName == 'OEE_FT' || tPageName == 'OEE_DS')
            ...
            let tTitle = ['OEE', 'OEE Target', 'AE', 'QE', 'OE', 'RE'];
            let tColor = ['#38AC26', 'red', 'blue', 'black', 'rgb(112,48,160)', 'rgb(239,99,229)'];
            let tValue = [[], [], [], [], [], []];
            let tSeries = [];
            ...
            let xTitle = JSON.parse(JSON.stringify(gPageObj.PageNameObj[tPageName].TitleStrArr));
            let tSeries = [];
            let tOtherArr = [0, 0, 0, 0, 0, 0, 0];
            let tTotalArr = [0, 0, 0, 0, 0, 0, 0];
            for (let i = 0, j = 0; i < tdata.length; i++) {
                if (data[i].toString().split(',')[1].trim() == '') {
                    continue;
                }
                let tIdx = tdata[i][2] - 1;
            ...
            let tColorArr = ['rgb(142,180,227)', 'blue', 'rgb(255,0,0)'];
            let xTitle = JSON.parse(JSON.stringify(gPageObj.PageNameObj[tPageName].TitleStrArr));
            let tSeries = [];
            let tDObj = {};
            ...
            let tColorArr = ['rgb(142,180,227)', 'blue'];
            let xTitle = JSON.parse(JSON.stringify(gPageObj.PageNameObj[tPageName].TitleStrArr));
            let tSeries = [];
            let tDObj = {};
            for (let i = 0; i < xTitle.length; i++) {
            ...
            option = {
                ...
                tooltip: {
                    trigger: 'item',
                    valueFormatter: (value) => (value + (tPageName != 'QE_RTAR' ? '%' : ''))
                },
                series: [
                    {
                        type: 'pie',
