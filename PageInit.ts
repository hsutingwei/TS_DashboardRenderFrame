...
class FormInf {
    ...
    TitleStrArr: Array<string> = [];
    /**紀錄多重欄位(真實的欄位型態) */
    TitleCell: string[][] = [];
    /**是否必填 */
    NecessaryArr: Array<boolean> = [];
    /**可否修改 */
    ModifiableArr: Array<boolean> = [];
    /**這次的搜尋結果 */
    FullData: string[][] = [];
...
class SearchOperation implements Search, ClickSearch {
    ...
    public static Search(tPageName?: string, JumPage?: number): void {
        ...
        if (PageNumber >= 1) {
            ...
            doAjax('Search', true, Query, function (FirstData: string[]) {
                ...
                TableObj.ajax = function (data: any, callback: any, settings: any) {
                    ...
                    gPageObj.PageNameObj[tmpPageName].AjaxStatus = doAjax2('Search', true, Query, function (result: string[]) {
                        ...
                        let so = new SearchOperation();
                        result = so.EditSearchResult(tmpPageName, result) as string[];
                        let returnData: tDataInf = {
                            draw: data.draw,
                            recordsTotal: AllResultCount,
                            recordsFiltered: AllResultCount,
            ...
            gPageObj.PageNameObj[tmpPageName].AjaxStatus = doAjax2('Search', true, Query, function (data: string[]) {
                let pm = new PageMake();
                let ps = new set.PageSet();
                let pt = new PageTool();
                let vd = new set.ValueDisplay();
...
export class PageMake implements PageRender {
    ...
    public CreatReadWriteTable(tPageName: string, data: Array<string> | { [key: string]: string }[], AttributeStr: string, TitleArr: Array<Array<string>>) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            for (let j = 0; j < tmpArr.length; j++) {
                ...
                let tClassName: string = 'cell_' + j.toString();
                let ClickHtml = '';
                let aPart: string = '';
                let MenuPart: string = '';
                let bPart: string = '';
     ...
     */
    public MakeWidthAttributeStr(tPageName: string, InputFieldName: string, StyleAttr: string, tMode: 'Read' | 'Write' | 'Search'): string {
        let ps = new set.PageSet();
        ...
        for (let i = 0; i < ValueIdArr.length; i++) {
            ...
            if (KeyValueArr[i] == '' && tListArr.length > 0 && tListArr[0].split(',')[0] != ''
                ...
                dc.DynamicInfObj[tPageName].InfluenceToFieldNames![tFieldNameArr[i]][ValueIdArr[i]].ValueByIdName?.forEach(function (item, idx) {
                    ...
                    let tmpIdx: number = Number(item.split('_')[1]);
                    let tDIdx: number = DefaultKey.indexOf(gPageObj.PageNameObj[tPageName].FieldArr[tmpIdx]);
                    (tmpKeyValue as string[]).push(DefaultValue[tDIdx]);
                });
            }
            else {
    ...
    */
    public IniteMenuBar(NeedCheckRight: boolean, DomId?: string, PageMode?: string) {
        doAjax('../Home/GetMenu' + (NeedCheckRight ? '2' : ''), true, { NeedCheckRight: NeedCheckRight ? '1' : '0', bu: PageMode }, function (data: MenuObj[]) {
            let MenuHtml: string = '';
...
export class PageTool {
        ...
        for (let i = 0; document.getElementById('field_' + i); i++) {
            if (ExceptforIdArr.indexOf('field_' + i) < 0 && document.getElementById('field_' + i)!.innerHTML.indexOf('<option') > -1) {
                ...
                if (DefaultIdx > -1) {
                    ...
                    if (Object.keys(dc.DynamicInfObj[tmpPageName].InfluenceToFieldNames?.[gPageObj.PageNameObj[tmpPageName].FieldArr[i]] || []).length > 0) {
                        let tDCHtml = '';
                        tDCHtml += dc.ReturnFunctionStr(tmpPageName, gPageObj.PageNameObj[tmpPageName].FieldArr[i], true);
                        for (let key in dc.DynamicInfObj[tmpPageName].InfluenceToFieldNames?.[gPageObj.PageNameObj[tmpPageName].FieldArr[i]]) {
                            KeyValueArr.push(DefaultValue[DefaultIdx]);
                            ValueIdArr.push(key);
     ...
     */
    public SetDefaultValue(DefaultObj: { DisplayArr: string[], DefaultKey: string[], DefaultValue: string[] }, UrlObj: { [key: string]: string }): { DisplayArr: string[], DefaultKey: string[], DefaultValue: string[] } {
